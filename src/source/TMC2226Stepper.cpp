...
TMC2226Stepper::TMC2226Stepper(Stream * SerialPort, float RS, uint8_t addr) :
	TMCStepper(RS),
	slave_address(addr)
	{
		HWSerial = SerialPort;
...
TMC2226Stepper::TMC2226Stepper(Stream * SerialPort, float RS, uint8_t addr, uint16_t mul_pin1, uint16_t mul_pin2) :
	TMC2226Stepper(SerialPort, RS)
	{
	...
	// Protected
	// addr needed for TMC2209
	TMC2226Stepper::TMC2226Stepper(uint16_t SW_RX_pin, uint16_t SW_TX_pin, float RS, uint8_t addr) :
		TMCStepper(RS),
	...
	void TMC2226Stepper::beginSerial(uint32_t baudrate) {
		if (SWSerial != nullptr)
		{
...
bool TMC2226Stepper::isEnabled() { return !enn() && toff(); }

uint8_t TMC2226Stepper::calcCRC(uint8_t datagram[], uint8_t len) {
	uint8_t crc = 0;
...
__attribute__((weak))
void TMC2226Stepper::preWriteCommunication() {
	if (HWSerial != nullptr) {
		if (sswitch != nullptr)
			sswitch->active();
	}
...
__attribute__((weak))
void TMC2226Stepper::preReadCommunication() {
	#if SW_CAPABLE_PLATFORM
		if (SWSerial != nullptr) {
			SWSerial->listen();
		} else
...
__attribute__((weak))
int16_t TMC2226Stepper::serial_read() {
	int16_t out = 0;
	#if SW_CAPABLE_PLATFORM
		if (SWSerial != nullptr) {
			out = SWSerial->read();
...
__attribute__((weak))
uint8_t TMC2226Stepper::serial_write(const uint8_t data) {
	int out = 0;;
	#if SW_CAPABLE_PLATFORM
		if (SWSerial != nullptr) {
...
__attribute__((weak))
void TMC2226Stepper::postWriteCommunication() {}

__attribute__((weak))
void TMC2226Stepper::postReadCommunication() {
	#if SW_CAPABLE_PLATFORM
		if (SWSerial != nullptr) {
			SWSerial->end();
		}
...
void TMC2226Stepper::write(uint8_t addr, uint32_t regVal) {
	uint8_t len = 7;
	addr |= TMC_WRITE;
	uint8_t datagram[] = {TMC2226_SYNC, slave_address, addr, (uint8_t)(regVal>>24), (uint8_t)(regVal>>16), (uint8_t)(regVal>>8), (uint8_t)(regVal>>0), 0x00};
...
uint64_t TMC2226Stepper::_sendDatagram(uint8_t datagram[], const uint8_t len, uint16_t timeout) {
...
uint32_t TMC2226Stepper::read(uint8_t addr) {
	constexpr uint8_t len = 3;
	addr |= TMC_READ;
	uint8_t datagram[] = {TMC2226_SYNC, slave_address, addr, 0x00};
	datagram[len] = calcCRC(datagram, len);
...
uint8_t TMC2226Stepper::IFCNT() {
	return read(IFCNT_t::address);
}

void TMC2226Stepper::SLAVECONF(uint16_t input) {
	SLAVECONF_register.sr = input&0xF00;
	write(SLAVECONF_register.address, SLAVECONF_register.sr);
}
uint16_t TMC2226Stepper::SLAVECONF() {
	return SLAVECONF_register.sr;
}
void TMC2226Stepper::senddelay(uint8_t B) 	{ SLAVECONF_register.senddelay = B; write(SLAVECONF_register.address, SLAVECONF_register.sr); }
uint8_t TMC2226Stepper::senddelay() 		{ return SLAVECONF_register.senddelay; }

void TMC2226Stepper::OTP_PROG(uint16_t input) {
	write(OTP_PROG_t::address, input);
}

uint32_t TMC2226Stepper::OTP_READ() {
	return read(OTP_READ_t::address);
}

uint32_t TMC2226Stepper::IOIN() {
	return read(TMC2226_n::IOIN_t::address);
}
bool TMC2226Stepper::enn()			{ TMC2226_n::IOIN_t r{0}; r.sr = IOIN(); return r.enn;		    }
bool TMC2226Stepper::ms1()			{ TMC2226_n::IOIN_t r{0}; r.sr = IOIN(); return r.ms1;		    }
bool TMC2226Stepper::ms2()			{ TMC2226_n::IOIN_t r{0}; r.sr = IOIN(); return r.ms2;		    }
...
void TMC2226Stepper::FACTORY_CONF(uint16_t input) {
	FACTORY_CONF_register.sr = input;
	write(FACTORY_CONF_register.address, FACTORY_CONF_register.sr);
}
void TMC2226Stepper::fclktrim(uint8_t B){ FACTORY_CONF_register.fclktrim = B; write(FACTORY_CONF_register.address, FACTORY_CONF_register.sr); }
void TMC2226Stepper::ottrim(uint8_t B)	{ FACTORY_CONF_register.ottrim = B; write(FACTORY_CONF_register.address, FACTORY_CONF_register.sr); }
...
uint32_t TMC2226Stepper::VACTUAL() {
	return VACTUAL_register.sr;
}

uint32_t TMC2226Stepper::PWM_SCALE() {
	return read(TMC2226_n::PWM_SCALE_t::address);
}
uint8_t TMC2226Stepper::pwm_scale_sum() {
	TMC2226_n::PWM_SCALE_t r{0};
	r.sr = PWM_SCALE();
	return r.pwm_scale_sum;
}

int16_t TMC2226Stepper::pwm_scale_auto() {
	TMC2226_n::PWM_SCALE_t r{0};
	r.sr = PWM_SCALE();
...
// R: PWM_AUTO
uint32_t TMC2226Stepper::PWM_AUTO() {
	return read(PWM_AUTO_t::address);
}
uint8_t TMC2226Stepper::pwm_ofs_auto()  { PWM_AUTO_t r{0}; r.sr = PWM_AUTO(); return r.pwm_ofs_auto; }
uint8_t TMC2226Stepper::pwm_grad_auto() { PWM_AUTO_t r{0}; r.sr = PWM_AUTO(); return r.pwm_grad_auto; }
