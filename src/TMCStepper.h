...
#include "source/TMC2226_bitfields.h"
...
public:
	...
	TMC5161Stepper(uint16_t pinCS, uint16_t pinMOSI, uint16_t pinMISO, uint16_t pinSCK, int8_t link_index = -1) : TMC5160Stepper(pinCS, pinMOSI, pinMISO, pinSCK, link_index) {}
	TMC5161Stepper(uint16_t pinCS, float RS, uint16_t pinMOSI, uint16_t pinMISO, uint16_t pinSCK, int8_t link_index = -1) : TMC5160Stepper(pinCS, RS, pinMOSI, pinMISO, pinSCK, link_index) {}
};
...
public:
	...
	TMC2208Stepper(Stream *SerialPort, float RS) : TMC2208Stepper(SerialPort, RS, TMC2208_SLAVE_ADDR)
	{
	}

	TMC2208Stepper(Uart *SerialPort, float RS) : TMC2208Stepper((Stream *)SerialPort, RS, TMC2208_SLAVE_ADDR)
	{
	}

#if SW_CAPABLE_PLATFORM
	TMC2208Stepper(uint16_t SW_RX_pin, uint16_t SW_TX_pin, float RS) : TMC2208Stepper(SW_RX_pin, SW_TX_pin, RS, TMC2208_SLAVE_ADDR)
	{
	}

	__attribute__((deprecated("Boolean argument has been deprecated and does nothing")))
	TMC2208Stepper(uint16_t SW_RX_pin, uint16_t SW_TX_pin, float RS, bool) : TMC2208Stepper(SW_RX_pin, SW_TX_pin, RS, TMC2208_SLAVE_ADDR){};
#else
...
{
public:
	TMC2209Stepper(Stream *SerialPort, float RS, uint8_t addr) : TMC2208Stepper(SerialPort, RS, addr) {}

#if SW_CAPABLE_PLATFORM
	TMC2209Stepper(uint16_t SW_RX_pin, uint16_t SW_TX_pin, float RS, uint8_t addr) : TMC2208Stepper(SW_RX_pin, SW_TX_pin, RS, addr)
	{
	}
#else
...
class TMC2226Stepper : public TMCStepper
{
public:
	...
	__attribute__((deprecated("Boolean argument has been deprecated and does nothing")))
	TMC2226Stepper(uint16_t SW_RX_pin, uint16_t SW_TX_pin, float RS, bool) : TMC2226Stepper(SW_RX_pin, SW_TX_pin, RS, TMC2226_SLAVE_ADDR){};
#else
	TMC2226Stepper(uint16_t, uint16_t, float) = delete;			 // Your platform does not currently support Software Serial
#endif
	void defaults();
	void push();
	void begin();
...
protected:
	INIT2226_REGISTER(GCONF){{.sr = 0}};
	INIT_REGISTER(SLAVECONF){{.sr = 0}};
	INIT_REGISTER(FACTORY_CONF){{.sr = 0}};
	INIT2226_REGISTER(VACTUAL){.sr = 0};
	INIT2226_REGISTER(CHOPCONF){{.sr = 0}};
	INIT2226_REGISTER(PWMCONF){{.sr = 0}};
	...
	const uint8_t slave_address;
	uint8_t calcCRC(uint8_t datagram[], uint8_t len);
	static constexpr uint8_t TMC2226_SYNC = 0x05,
							 TMC2226_SLAVE_ADDR = 0x00;
	static constexpr uint8_t replyDelay = 2;
	static constexpr uint8_t abort_window = 5;
	static constexpr uint8_t max_retries = 2;
